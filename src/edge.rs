...
async fn edge_post(url: &str, body: &str) -> Result<String> {
    let req = build_request("POST", url, Some(body.to_string()))?;
    execute(req).await
}

async fn edge_get(url: &str) -> Result<String> {
    let req = build_request("GET", url, None)?;
    execute(req).await
}

async fn edge_delete(url: &str) -> Result<String> {
    let req = build_request("DELETE", url, None)?;
    execute(req).await
}
...
impl EdgeClient {
    async fn enqueue_inner(&self, job_type: &str, args: JsValue) -> Result<JsValue> {
        ...
        let resp_text = edge_post(&url, &body).await?;
    ...
    async fn get_job_inner(&self, id: &str) -> Result<JsValue> {
        ...
        let resp_text = edge_get(&url).await?;
        let resp: JobResponse = serde_json::from_str(&resp_text)?;
        serde_wasm_bindgen::to_value(&resp.job)
            .map_err(|e| OjsWasmError::Serialization(e.to_string()))
    }
    ...
    async fn cancel_job_inner(&self, id: &str) -> Result<JsValue> {
        let url = format!("{}/jobs/{}", self.base_url, id);
        let resp_text = edge_delete(&url).await?;
        let resp: JobResponse = serde_json::from_str(&resp_text)?;
        serde_wasm_bindgen::to_value(&resp.job)
            .map_err(|e| OjsWasmError::Serialization(e.to_string()))
    }
...
// ===========================================================================
// CloudflareClient — Cloudflare Workers
// ===========================================================================
