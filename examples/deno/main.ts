...
import init, { DenoClient, chain, group, batch } from '@openjobspec/wasm';
...
console.log('OJS Deno server starting on :3000');
...
Deno.serve({ port: 3000 }, async (req: Request) => {
  ...
  try {
    ...
    // POST /workflow/batch
    if (url.pathname === '/workflow/batch' && req.method === 'POST') {
      const body = await req.json();
      const definition = batch(body.jobs, body.callbacks);
      const status = await client.workflow(definition);
      return Response.json(status, { status: 201 });
    }
    ...
    return new Response('OJS Deno Deploy — see /health, /enqueue, /job/:id', {
      status: 200,
    });
  } catch (e) {
