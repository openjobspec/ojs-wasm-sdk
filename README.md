...
- **Size-optimized release profile** — `opt-level = "z"`, LTO, symbol stripping,
  `panic = "abort"`, and converged `wasm-opt -Oz`
...
| Metric | 0.5.0 default build |
|--------|---------------------|
| `.wasm` binary (`-Oz`, LTO, `wasm-opt`) | 370,278 bytes |
| Deterministic gzip (`gzip -cn`) | 149,733 bytes |
| JS glue code | ~116 KB |
| `init()` time (cold) | ~5-15 ms |
| `enqueue()` overhead vs raw fetch | < 1 ms |
| JSON serialization throughput | >100K ops/sec |

The release profile uses `opt-level = "z"`, LTO, one codegen unit,
`panic = "abort"`, symbol stripping, and converged `wasm-opt -Oz`. CI enforces
a **151,552-byte (148 KiB) deterministic-gzip regression budget**, 1.2% above
the measured 0.5.0 artifact. Exceeding the budget fails the build.

Measured feature builds show why this cannot be closed with linker flags alone:
the no-default core is already about **270 KB raw / 110 KB gzip**. Adding
Service Worker support reaches ~124 KB gzip, encryption ~122 KB, schema
validation ~117 KB, and all edge clients ~118 KB; the ABI-compatible default
combination is ~150 KB. `twiggy` attributes about 77 KB raw in the named
pre-bindgen build to 62 monomorphized `future_to_promise` wrappers generated by
the many public async methods, in addition to serde/JSON and allocator code.

The former 50 KiB threshold was below the 110 KB no-default core and could not
measure regressions in the advertised package. Reaching 50 KiB still requires
a product/API decision: publish a minimal default, split runtimes into separate
packages, or redesign repeated async WASM bindings. Version 0.5.0 preserves the
advertised default behavior and replaces that arbitrary threshold with a tight,
failing-on-regression budget. See [Bundle Size Budget](docs/BUNDLE_SIZE.md) for
measurements and optimization trials.

To check bundle size locally:

```bash
make build-release
gzip -cn pkg/ojs_wasm_sdk_bg.wasm | wc -c
```
...
| Feature | Description |
|---------|-------------|
| `edge_cloudflare` | Exports `CloudflareClient`, `KVNamespaceRef`, and `D1DatabaseRef` |
| `edge_deno` | Exports `DenoClient` |
| `edge_vercel` | Exports `VercelEdgeClient` |
| `edge_all` | Enables all three edge runtime export groups |
| `service_worker` | Exports `ServiceWorkerClient` and durable Background Sync helpers |
| `encryption` | Exports synchronous AES-256-GCM helpers |
| `schema` | Exports `SchemaValidator` |

Default features are `edge_all`, `service_worker`, `encryption`, and `schema`
to preserve the 0.4.0 JavaScript/WASM ABI. With `default-features = false`,
only the generic browser/edge core and always-on extension modules are built;
each feature above adds only its documented exports.
...
4. Run `make build-release` and verify the 151,552-byte deterministic-gzip budget
